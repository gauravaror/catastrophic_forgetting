...
exper=$2
exec_python="python"
find $path -name '*ngram*' | while read p; do $exec_python aggregator.py --path $p  --store_path $path/evaluate_csv_agg --output csv --store_df --allowed_keys standard_evaluate --allowed_keys evaluate --allowed_keys restore_checkpoint --allowed_keys forgetting_metric --allowed_keys task_diagnostics; done > ${exper}_agg.log
