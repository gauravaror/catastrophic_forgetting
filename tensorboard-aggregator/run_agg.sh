...
path=$1
exper=$2
find $path -name '*_exper__*' | while read p; do python aggregator.py --path $p  --store_path $path/evaluate_csv_agg --output csv --store_df --allowed_keys standard_evaluate --allowed_keys evaluate --allowed_keys restore_checkpoint --allowed_keys forgetting_metric; done > ${exper}_agg.log
