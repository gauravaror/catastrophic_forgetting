...
@Seq2VecEncoder.register("cnn_internal")
class CnnEncoder(Seq2VecEncoder):
    ...
    def __init__(self,
        ...
        for l in range(self._num_layers-1):
            for i, conv_layer in enumerate(self._convolution_layers2[l]):
                self.add_module('conv_layer%d_%d' % (l,i), conv_layer)

        maxpool_output_dim = self._num_filters
        if self._output_dim:
